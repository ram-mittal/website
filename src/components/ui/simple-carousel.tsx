...
const CarouselDots = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "backdrop-blur-xs absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-2 rounded-full border border-white/20 bg-white/50 p-2 transition-all duration-300 hover:scale-105 dark:border-black/10 dark:bg-black/40",
          className,
        )}
        ref={ref}
        {...props}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      className={cn(
        "backdrop-blur-xs absolute grid size-8 place-content-center rounded-full border border-white/20 bg-white/50 transition-all active:scale-75 dark:border-black/10 dark:bg-black/40",
        "transform-gpu hover:scale-105 disabled:cursor-not-allowed disabled:bg-white/10 dark:disabled:bg-black/10",
        orientation === "horizontal"
          ? "left-6 top-1/2 -translate-y-1/2"
          : "left-1/2 top-6 -translate-x-1/2 rotate-90",
        className,
      )}
